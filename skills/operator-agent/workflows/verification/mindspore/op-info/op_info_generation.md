...
> The current ST uses the **op info testing framework**. The core operation is registering OpInfo in `op_database.py`. Writing separate standalone test files is not allowed. Use the existing `op_info.py`, `op_database.py`, and frontend op-info tests in the target MindSpore workspace as the source of truth.
...
### Zero-Bit Accuracy Validation (when needed)
...
### Memory Alignment Validation (when needed)
